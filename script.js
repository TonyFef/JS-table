...
onFilterActive = () => {
    ...
    const parentTh = document.querySelector(".parent");
    parentTh.style.display = "none";
};
...
startButton.addEventListener("click", () => {
    ...
    for (let elem of data) {
        ...
        element.innerHTML = `<td>${elem.id}</td><td>${elem.name}</td><td>${elem.email}</td><td class='element'>${elem.balance}</td>`;
        tbody.append(element);
    }
    const parentTh = document.querySelector(".parent");
    parentTh.style.display = "block";
    ...
    for (let elem of tr) {
        elem.addEventListener(
            ...
            () => {
                ...
                parent.innerHTML = `${elem.parentId}`;

                let parentNone = document.createElement("td");
                parentNone.innerHTML = `None`;

                if (elem.parentId == 0) {
                    elem.append(parentNone);
                } else {
                    elem.append(parent);
                }
